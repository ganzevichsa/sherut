...
<div class="row item-ability">
            ...
            <div class="col-sm-12">
                <select name="ability_id[]" class="form-control">
                    @foreach(\App\Models\QuizMaterials::get() as $ability)
                        <option value="{{ $ability->id }}" @if($ability->id == $data->ability_id) selected @endif>{{ $ability->title }}</option>
                    @endforeach
                </select>
            </div>
        </div>
    ...
    <div class="col-md-3">
        <div class="form-group">
            ...
            <div class="col-sm-12">
                <input type="text" name="ability_value[]" class="form-control" value="{{ $data->ability_value }}">
            </div>
        </div>
    </div>
    <div class="col-md-3">
    ...
    border-radius: .25rem;">
                    ...
                    <input type="text" name="ability_percent[]" class="form-control" value="{{ $data->ability_percent }}">
                </div>
            </div>
        </div>
    </div>
